...
func (tw *TimeWheel) checkAndRunTask() {
	...
	tw.taskCircleRecords.Range(func(key interface{}, value interface{}) bool {
		tw.taskCircleRecords.Delete(key)
		value.(*list.Element).Value.(*Task).circle--
		return true
	})
	// 轮盘前进一步
...
func (tw *TimeWheel) addTask(task *Task, byInterval bool) {
	...
	if task.pos == tw.currentPos && circle != 0 {
		tw.taskCircleRecords.Store(task.key, element)
	}
}
