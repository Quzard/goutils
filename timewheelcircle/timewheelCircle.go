...
// TimeWheelCircle 核心结构体
type TimeWheelCircle struct {
	// 时间轮盘的精度   一个刻度多少时间
...
type Task struct {
	...
	// 任务需要在轮盘走多少圈才能执行
	circle int
	// 任务需要执行的Job，优先级高于TimeWheel中的Job, 一个函数
	job Job
	// 任务需要执行的次数，如果需要一直执行，设置成-1
	times int
}

var tw *TimeWheelCircle
var once sync.Once
...
func CreateTimeWheel(interval time.Duration, slotNums int, job Job) *TimeWheelCircle {
	once.Do(func() {
...
// GetTimeWheel 返回tw对象
func GetTimeWheel() *TimeWheelCircle {
	return tw
}

// New 初始化一个TimeWheel对象
func New(interval time.Duration, slotNums int, job Job) *TimeWheelCircle {
...
// Stop 关闭时间轮盘
func (tw *TimeWheelCircle) Stop() {
	tw.stopChannel <- true
	tw.isRunning = false
}

// IsRunning 检查一下时间轮盘的是否在正常运行
func (tw *TimeWheelCircle) IsRunning() bool {
	return tw.isRunning
}
...
func (tw *TimeWheelCircle) AddTask(interval time.Duration, key interface{}, createdTime time.Time, times int, job Job) error {
...
func (tw *TimeWheelCircle) RemoveTask(key interface{}) error {
	if key == nil {
		return nil
	}
...
func (tw *TimeWheelCircle) initSlots() {
...
func (tw *TimeWheelCircle) checkAndRunTask() {
...
func (tw *TimeWheelCircle) addTask(task *Task, byInterval bool) {
	var pos, circle int
	if byInterval {
...
// 删除任务的内部函数
func (tw *TimeWheelCircle) removeTask(task *Task) {
	// 从map结构中删除
...
func (tw *TimeWheelCircle) getPosAndCircleByInterval(d time.Duration) (int, int) {
...
func (tw *TimeWheelCircle) getPosAndCircleByCreatedTime(createdTime time.Time, d time.Duration, key interface{}) (int, int) {
